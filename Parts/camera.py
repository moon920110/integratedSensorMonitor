...
class VideoRecorder():
    def __init__(self, camera_index=0):
        ...
        self.init_conection(camera_index)

        self.video_frames = []
    
    def init_conection(self, camera_index=0):
        self.vid_capture = self.__open_camera(camera_index)
        if self.vid_capture == None:
    ...
    def save(self,start_frame=0, end_frame=-1, file_name='temp', file_path='./'):
        ...
        vid_recorder = cv2.VideoWriter(file_path+file_name, vid_cod, self.vid_attribute['fps'], (self.vid_attribute['width'],self.vid_attribute['height']))
